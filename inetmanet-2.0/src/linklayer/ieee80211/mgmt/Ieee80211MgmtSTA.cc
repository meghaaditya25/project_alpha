...
#define MK_DATA_TIMEOUT       6

#define MAX_DATAS_MISSED 3.5  // data lost timeout, in data intervals (doesn't need to be integer)
...
std::ostream& operator<<(std::ostream& os, const Ieee80211MgmtSTA::AssociatedAPInfo& assocAP)
{
    os << "AP addr=" << assocAP.address
       ...
       << " ssid=" << assocAP.ssid
       << " beaconIntvl=" << assocAP.dataInterval
...
void Ieee80211MgmtSTA::handleTimer(cMessage *msg)
{
    ...
    else if (msg->getKind()==MK_DATA_TIMEOUT)
    {
...
void Ieee80211MgmtSTA::beaconLost()
{
    ...
    nb->fireChangeNotification(NF_L2_DATA_LOST, myIface);
}
...
void Ieee80211MgmtSTA::receiveChangeNotification(int category, const cObject *details)
{
    ...
    else if (category==NF_LINK_FULL_PROMISCUOUS)
    {
        ...
        Ieee80211BeaconFrame *data= (check_and_cast<Ieee80211BeaconFrame *>(frame));
        APInfo *ap = lookupAP(data->getTransmitterAddress());
        if (ap)
...
bool Ieee80211MgmtSTA::scanNextChannel()
{
    ...
    if (scanning.activeScan)
    {
        ...
        scheduleAt(simTime()+scanning.probeDelay, new cMessage("TrustValue", MK_SCAN_SENDPROBE));
...
void Ieee80211MgmtSTA::sendScanConfirm()
{
    ...
    for (int i=0; i<n; i++, it++)
    {
        ...
        bss.setBeaconInterval(ap->dataInterval);
...
void Ieee80211MgmtSTA::disassociate()
{
    ...
    ASSERT(isAssociated);
    isAssociated = false;
    delete cancelEvent(assocAP.dataTimeoutMsg);
    assocAP.dataTimeoutMsg = NULL;
...
void Ieee80211MgmtSTA::handleAssociationResponseFrame(Ieee80211AssociationResponseFrame *frame)
{
    ...
    else
    {
        ...
        assocAP.dataTimeoutMsg = new cMessage("dataTimeout", MK_DATA_TIMEOUT);
        scheduleAt(simTime()+MAX_DATAS_MISSED*assocAP.dataInterval, assocAP.dataTimeoutMsg);
    }
...
{
    EV << "Received Data frame\n";
    storeAPInfo(frame->getTransmitterAddress(), frame->getBody());

    // if it is out associate AP, restart data timeout
    if (isAssociated && frame->getTransmitterAddress()==assocAP.address)
    {
        EV << "Data is from associated AP, restarting data timeout timer\n";
        ASSERT(assocAP.dataTimeoutMsg!=NULL);
        cancelEvent(assocAP.dataTimeoutMsg);
        scheduleAt(simTime()+MAX_DATAS_MISSED*assocAP.dataInterval, assocAP.dataTimeoutMsg);
...
void Ieee80211MgmtSTA::storeAPInfo(const MACAddress& address, const Ieee80211BeaconFrameBody& body)
{
    ...
    ap->dataInterval = body.getBeaconInterval();
