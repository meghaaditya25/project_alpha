...
void Radio::handleLowerMsgStart(AirFrame* airframe)
{
    ...
    else
    {
        ...
        if (snrInfo.ptr != NULL)
        {
            ...
            EV << "Discover the path to the Destination node\n";
            addNewSnr();
        }
...
void Radio::handleLowerMsgEnd(AirFrame * airframe)
{
    ...
    {
        EV << "Each node has to learn the shortest path to the nearest landmark node....\n";
    ...
    if (BASE_NOISE_LEVEL < receptionThreshold && rs.getState() == RadioState::RECV && snrInfo.ptr == NULL)
    {
        ...
        EV << "Location-independent routing in the distributed network\n";
        setRadioState(RadioState::IDLE);
    }
}
