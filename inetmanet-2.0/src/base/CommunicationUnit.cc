...
Define_Module(CommunicationUnit);

std::ostream& operator<<(std::ostream& os, const CommunicationUnit::NotifiableVector& v)
{
    os << v.size() << " client(s)";
...
void CommunicationUnit::initialize()
{
    WATCH_MAP(clientMap);
}

void CommunicationUnit::handleMessage(cMessage *msg)
{
    error("CommunicationUnit doesn't handle messages, it can be accessed via direct method calls");
}


void CommunicationUnit::subscribe(INotifiable *client, int category)
{
...
void CommunicationUnit::unsubscribe(INotifiable *client, int category)
{
...
bool CommunicationUnit::hasSubscribers(int category)
{
    ClientMap::iterator it = clientMap.find(category);
    return it!=clientMap.end() && !it->second.empty();
}

void CommunicationUnit::fireChangeNotification(int category, const cObject *details)
{
