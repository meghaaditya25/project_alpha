...
private:
  ...
  public:
    ...
    class APInfo : public cPolymorphic
    {
      public:
        ...
        APInfo()
        {
            channel=-1; dataInterval=rxPower=0; authSeqExpected=-1;
        ...
        APInfo(const APInfo& ap_info)
        {
            ...
            this->dataInterval = ap_info.dataInterval;
    ...
    class AssociatedAPInfo : public APInfo
    {
      public:
        int receiveSequence;
        bool isAssociated;
        cMessage *dataTimeoutMsg;
        cMessage *assocTimeoutMsg; // if non-NULL: association is in progress

        AssociatedAPInfo() : APInfo() {receiveSequence=0; isAssociated = false; dataTimeoutMsg=NULL; assocTimeoutMsg=NULL;}
        AssociatedAPInfo(const AssociatedAPInfo& assoc_ap) : APInfo(assoc_ap) {receiveSequence=0; dataTimeoutMsg=NULL;}
  ...
  protected:
    ...
    double max_beacons_missed; // number of max data missed to notify the agent about beacon_lost event
  ...
  protected:
    ...
    /** Stores AP info received in a data or probe response */
