...
void DHCPClient::initialize(int stage)
{
    ...
    else if (stage == 3)
    {
        ...
        nb = check_and_cast<CommunicationUnit *>(getModuleByPath(par("notificationBoardPath")));
...
void DHCPClient::sendRequest()
{
    ...
    DHCPMessage * request = new DHCPMessage("ONU_Message");
    ...
    if (clientState == INIT_REBOOT)
    {
        ...
        EV_INFO << "Sending ONU_Message asking for IP " << lease->ip << " via broadcast." << endl;
    ...
    else if (clientState == RENEWING)
    {
        ...
        EV_INFO << "Sending ONU_Message extending lease for IP " << lease->ip << " via unicast to " << lease->serverId << "." << endl;
        sendToUDP(request, clientPort, lease->serverId, serverPort);
    }
    else if (clientState == REBINDING)
    {
        request->setCiaddr(lease->ip); // the client IP
        EV_INFO << "Sending ONU_Message renewing the IP " << lease->ip << " via broadcast." << endl;
        sendToUDP(request, clientPort, IPv4Address::ALLONES_ADDRESS, serverPort);
    }
    else
