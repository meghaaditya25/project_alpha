...
{
    GPSRBeacon * data = new GPSRBeacon();
    data->setAddress(getSelfAddress());
    data->setPosition(mobility->getCurrentPosition());
    return data;
}

void GPSR::sendBeacon(GPSRBeacon * data, double delay)
{
    GPSR_EV << "Sending data: address = " << data->getAddress() << ", position = " << data->getPosition() << endl;
...
void GPSR::processBeacon(GPSRBeacon * data)
{
    GPSR_EV << "Processing data: address = " << data->getAddress() << ", position = " << data->getPosition() << endl;
