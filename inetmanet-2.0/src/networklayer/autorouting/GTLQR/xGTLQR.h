...
#ifndef __INET__GTLQR_H_
#define __INET__GTLQR_H_
...
/**
 * This class provides Dynamic MANET On-demand (GTLQR also known as AODVv2) Routing
 * based on the IETF draft at http://tools.ietf.org/html/draft-ietf-manet-GTLQR-24.
 *
 ...
 *    Allow intermediate GTLQR routers to reply with RREP.
 *  - 13.6. Message Aggregation
 *    RFC5148 add jitter to broadcasts
 */
class INET_API xGTLQR : public cSimpleModule, public ILifecycle, public INotifiable, public INetfilter::IHook
{
  private:
    ...
    std::map<IPv4Address, GTLQRSequenceNumber> targetAddressToSequenceNumber;
  ...
  private:
    ...
    void sendGTLQRPacket(GTLQRPacket * packet, const InterfaceEntry * interfaceEntry, const IPv4Address & nextHop, double delay);
    void processGTLQRPacket(GTLQRPacket * packet);
    ...
    int getLinkCost(const InterfaceEntry * interfaceEntry, GTLQRMetricType metricType);
    ...
    GTLQRRouteState getRouteState(SKHData * routeData);
