...
std::ostream& operator<<(std::ostream& os, const AF_BF::fec_bind_t& f)
{
    os << "fecid=" << f.fecid << "  peer=" << f.peer << " label=" << f.label;
    return os;
}

bool fecPrefixCompare(const AF_BF::fec_t& a, const AF_BF::fec_t& b)
{
    return a.length > b.length;
}

std::ostream& operator<<(std::ostream& os, const AF_BF::fec_t& f)
{
    os << "fecid=" << f.fecid << "  addr=" << f.addr << "  length=" << f.length << "  nextHop=" << f.nextHop;
    return os;
}

std::ostream& operator<<(std::ostream& os, const AF_BF::pending_req_t& r)
{
    os << "fecid=" << r.fecid << "  peer=" << r.peer;
    return os;
}

std::ostream& operator<<(std::ostream& os, const AF_BF::peer_info& p)
{
...
void AF_BF::initialize(int stage)
{
    ...
    else if (stage == 3)
    {
        ...
        sendHelloMsg = new cMessage("AF_BFSendHello");
        ...
        EV << "Starting to listen on port " << AF_BF_PORT << " for incoming AF_BF sessions\n";
        serverSocket.setOutputGate(gate("tcpOut"));
        serverSocket.readDataTransferModePar(*this);
        serverSocket.bind(AF_BF_PORT);
        serverSocket.listen();
...
void AF_BF::handleMessage(cMessage *msg)
{
    if (!isNodeUp())
        throw cRuntimeError("AF_BF is not running");
    EV << "Received: (" << msg->getClassName() << ")" << msg->getName() << "\n";
    if (msg==sendHelloMsg)
    {
        // every AF_BF capable router periodically sends HELLO messages to the
        // "all routers in the sub-network" multicast address
        EV << "Multicasting AF_BF Hello to neighboring routers\n";
    ...
    else if (msg->isSelfMessage())
    {
        ...
        {
            processNOTIFICATION(check_and_cast<AF_BFNotify*>(msg));
        }
    }
    else if (!strcmp(msg->getArrivalGate()->getName(), "udpIn"))
    {
        // we can only receive AF_BF Hello from UDP (everything else goes over TCP)
        processAF_BFHello(check_and_cast<AF_BFHello *>(msg));
    }
...
bool AF_BF::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void AF_BF::sendToPeer(IPv4Address dest, cMessage *msg)
{
    getPeerSocket(dest)->send(msg);
}

void AF_BF::sendMappingRequest(IPv4Address dest, IPv4Address addr, int length)
{
    AF_BFLabelRequest *requestMsg = new AF_BFLabelRequest("Lb-Req");
    requestMsg->setByteLength(AF_BF_HEADER_BYTES); // FIXME find out actual length
...
void AF_BF::updateFecListEntry(AF_BF::fec_t oldItem)
{
    ...
    for (uit = fecUp.begin(); uit != fecUp.end();)
    {
        ...
        if (ER)
        {
            ...
            uit->label = lt->installLibEntry(uit->label, inInterface, outLabel, outInterface, AF_BF_USER_TRAFFIC);
...
void AF_BF::updateFecList(IPv4Address nextHop)
{
    FecVector::iterator it;
...
void AF_BF::sendHelloTo(IPv4Address dest)
{
    AF_BFHello *hello = new AF_BFHello("AF_BF-Hello");
    hello->setByteLength(AF_BF_HEADER_BYTES);
    hello->setType(HELLO);
    ...
    //hello->setRbit(...);
    //hello->setTbit(...);
    hello->addPar("color") = AF_BF_HELLO_TRAFFIC;

    if (dest.isMulticast())
    {
        for (int i = 0; i < (int)udpSockets.size(); ++i)
        {
            AF_BFHello *msg = i== (int)udpSockets.size() - 1 ? hello : hello->dup();
            udpSockets[i].sendTo(msg, dest, AF_BF_PORT);
        }
    }
    else
        udpSocket.sendTo(hello, dest, AF_BF_PORT);
}

void AF_BF::processHelloTimeout(cMessage *msg)
{
    // peer is gone
...
void AF_BF::processAF_BFHello(AF_BFHello *msg)
{
    ...
    EV << "Received AF_BF Hello from " << peerAddr << ", ";
...
void AF_BF::openTCPConnectionToPeer(int peerIndex)
{
    ...
    socket->connect(myPeers[peerIndex].peerIP, AF_BF_PORT);
}

void AF_BF::processMessageFromTCP(cMessage *msg)
{
...
void AF_BF::socketEstablished(int, void *yourPtr)
{
    ...
    // FIXME start AF_BF session setup (if we're on the active side?)
}

void AF_BF::socketDataArrived(int, void *yourPtr, cPacket *msg, bool)
{
    peer_info& peer = myPeers[(long)yourPtr];
    EV << "Message arrived over TCP from peer " << peer.peerIP << "\n";

    delete msg->removeControlInfo();
    processAF_BFPacketFromTCP(check_and_cast<AF_BFPacket *>(msg));
}

void AF_BF::socketPeerClosed(int, void *yourPtr)
{
    peer_info& peer = myPeers[(long)yourPtr];
...
void AF_BF::socketClosed(int, void *yourPtr)
{
    peer_info& peer = myPeers[(long)yourPtr];
...
void AF_BF::socketFailure(int, void *yourPtr, int code)
{
    peer_info& peer = myPeers[(long)yourPtr];
...
void AF_BF::processAF_BFPacketFromTCP(AF_BFPacket *AF_BFPacket)
{
    switch (AF_BFPacket->getType())
    {
    case HELLO:
        error("Received AF_BF HELLO over TCP (should arrive over UDP)");
        break;

    case ADDRESS:
        // processADDRESS(AF_BFPacket);
        error("Received AF_BF ADDRESS message, unsupported in this version");
        break;

    case ADDRESS_WITHDRAW:
        // processADDRESS_WITHDRAW(AF_BFPacket);
        error("AF_BF PROC DEBUG: Received AF_BF ADDRESS_WITHDRAW message, unsupported in this version");
        break;

    case LABEL_MAPPING:
        processLABEL_MAPPING(check_and_cast<AF_BFLabelMapping *>(AF_BFPacket));
        break;

    case LABEL_REQUEST:
        processLABEL_REQUEST(check_and_cast<AF_BFLabelRequest *>(AF_BFPacket));
        break;

    case LABEL_WITHDRAW:
        processLABEL_WITHDRAW(check_and_cast<AF_BFLabelMapping *>(AF_BFPacket));
        break;

    case LABEL_RELEASE:
        processLABEL_RELEASE(check_and_cast<AF_BFLabelMapping *>(AF_BFPacket));
        break;

    case NOTIFICATION:
        processNOTIFICATION(check_and_cast<AF_BFNotify*>(AF_BFPacket));
        break;

    default:
        error("AF_BF PROC DEBUG: Unrecognized AF_BF Message Type, type is %d", AF_BFPacket->getType());
        break;
    }
}

IPv4Address AF_BF::locateNextHop(IPv4Address dest)
{
...
IPv4Address AF_BF::findPeerAddrFromInterface(std::string interfaceName)
{
    int i = 0;
    int k = 0;
...
std::string AF_BF::findInterfaceFromPeerAddr(IPv4Address peerIP)
{
/*
    int i;
    for (unsigned int i=0;i<myPeers.size();i++)
...
//bool AF_BF::matches(const FEC_TLV& a, const FEC_TLV& b)
//{
//  return b.addr.prefixMatches(a, b.length);
//}

AF_BF::FecBindVector::iterator AF_BF::findFecEntry(FecBindVector& fecs, int fecid, IPv4Address peer)
{
...
AF_BF::FecVector::iterator AF_BF::findFecEntry(FecVector& fecs, IPv4Address addr, int length)
{
    FecVector::iterator it;
    for (it = fecs.begin(); it != fecs.end(); it++)
    {
...
void AF_BF::sendNotify(int status, IPv4Address dest, IPv4Address addr, int length)
{
    // Send NOTIFY message
    AF_BFNotify *lnMessage = new AF_BFNotify("Lb-Notify");
    lnMessage->setByteLength(AF_BF_HEADER_BYTES); // FIXME find out actual length
...
void AF_BF::sendMapping(int type, IPv4Address dest, int label, IPv4Address addr, int length)
{
    // Send LABEL MAPPING downstream
    AF_BFLabelMapping *lmMessage = new AF_BFLabelMapping("Lb-Mapping");
    lmMessage->setByteLength(AF_BF_HEADER_BYTES); // FIXME find out actual length
...
void AF_BF::processNOTIFICATION(AF_BFNotify *packet)
{
...
void AF_BF::processLABEL_REQUEST(AF_BFLabelRequest *packet)
{
    FEC_TLV fec = packet->getFec();
    IPv4Address srcAddr = packet->getSenderAddress();
...
void AF_BF::processLABEL_RELEASE(AF_BFLabelMapping *packet)
{
    FEC_TLV fec = packet->getFec();
...
void AF_BF::processLABEL_WITHDRAW(AF_BFLabelMapping *packet)
{
    FEC_TLV fec = packet->getFec();
    int label = packet->getLabel();
    IPv4Address fromIP = packet->getSenderAddress();
...
void AF_BF::processLABEL_MAPPING(AF_BFLabelMapping *packet)
{
    FEC_TLV fec = packet->getFec();
    int label = packet->getLabel();
    IPv4Address fromIP = packet->getSenderAddress();
    ...
    for (pit = pending.begin(); pit != pending.end();)
    {
        ...
        newItem.label = lt->installLibEntry(-1, inInterface, outLabel, outInterface, AF_BF_USER_TRAFFIC);
        fecUp.push_back(newItem);
...
int AF_BF::findPeer(IPv4Address peerAddr)
{
...
TCPSocket *AF_BF::findPeerSocket(IPv4Address peerAddr)
{
    // find peer in table and return its socket
    int i = findPeer(peerAddr);
    if (i==-1 || !(myPeers[i].socket) || myPeers[i].socket->getState()!=TCPSocket::CONNECTED)
        return NULL; // we don't have an AF_BF session to this peer
    return myPeers[i].socket;
}

TCPSocket *AF_BF::getPeerSocket(IPv4Address peerAddr)
{
    TCPSocket *sock = findPeerSocket(peerAddr);
    ASSERT(sock);
    if (!sock)
        error("No AF_BF session to peer %s yet", peerAddr.str().c_str());
    return sock;
}

bool AF_BF::lookupLabel(IPv4Datagram *ipdatagram, LabelOpVector& outLabel, std::string& outInterface, int& color)
{
    ...
    // AF_BF traffic (both discovery...
    if (protocol == IP_PROT_UDP && check_and_cast<UDPPacket*>(ipdatagram->getEncapsulatedPacket())->getDestinationPort() == AF_BF_PORT)
        return false;

    // ...and session)
    if (protocol == IP_PROT_TCP && check_and_cast<TCPSegment*>(ipdatagram->getEncapsulatedPacket())->getDestPort() == AF_BF_PORT)
        return false;
    if (protocol == IP_PROT_TCP && check_and_cast<TCPSegment*>(ipdatagram->getEncapsulatedPacket())->getSrcPort() == AF_BF_PORT)
        return false;
...
void AF_BF::receiveChangeNotification(int category, const cObject *details)
{
    Enter_Method_Silent();
...
void AF_BF::announceLinkChange(int tedlinkindex)
{
    TEDChangeInfo d;
