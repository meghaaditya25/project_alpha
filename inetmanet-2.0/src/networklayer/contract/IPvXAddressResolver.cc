...
CommunicationUnit *IPvXAddressResolver::notificationBoardOf(cModule *host)
{
    // find CommunicationUnit
    cModule *mod = host->getSubmodule("notificationBoard");
    if (!mod)
        throw cRuntimeError("IPvXAddressResolver: CommunicationUnit not found as submodule "
                  " notificationBoard' in host/router `%s'", host->getFullPath().c_str());

    return check_and_cast<CommunicationUnit *>(mod);
}
...
CommunicationUnit *IPvXAddressResolver::findCommunicationUnitOf(cModule *host)
{
    cModule *mod = host->getSubmodule("notificationBoard");
    return dynamic_cast<CommunicationUnit *>(mod);
}
