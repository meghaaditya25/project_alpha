...
void Ieee802154Mac::commonInitialize()
{
    ...
    registerInterface();
     ...
     mLowerLayerOut = findGate("lowerLayerOut");
           ...
           // get a pointer to the CommunicationUnit modul
     mpNb = CommunicationUnitAccess().get();
...
void Ieee802154Mac::initialize(int stage)
{
    ...
    if (0 == stage)
    {
        ...
        if (mpib.macBeaconOrder >15 || mpib.macSuperframeOrder > mpib.macBeaconOrder)
            ...
            error("[MAC]: non-data mode (BO = 15) has not been tested!");
...
void Ieee802154Mac::finish()
{
    ...
    recordScalar("num of DATA pkts sent",             numTxBcnPkt);
    ...
    recordScalar("num of DATA pkts received",     numRxBcnPkt);
    recordScalar("num of DATA pkts lost",     numLostBcn);
...
void Ieee802154Mac::startPANCoor()
{
    ...
    startBcnTxTimer(true);              // start to transmit my first data immediately
}

void Ieee802154Mac::startDevice()
{
    // now only star topology, only PAN coordinator transmits data
    mpib.macBeaconOrder     = 15;
    mpib.macSuperframeOrder     = 15;
    mpib.macAssociationPermit   = false;

    // open radio receiver, waiting for first data's arrival
...
{
    EV << "[MAC]: starting processing received Data frame" << endl;
    ...
    //calculate the time when the first bit of the data was received
    ...
    EV << "The first bit of this data was received by PHY layer at " << bcnRxTime << endl;
    ...
    if (notAssociated) // this is my first rxed data, associate with this one,
    {
        ...
        EV << "This is my first data, associate with it" << endl;
#if OMNETPP_VERSION >= 0x403
        ...
        startBcnRxTimer();              // start tracking data, always on
        // start sending data from here, if I want to be a coordinator
        /*
    ...
    // can start my GTS timer only after receiving the second data
    ...
    //CSMA-CA may be waiting for the new data
    if (backoffStatus == 99)
        csmacaTrxBeacon('r');
...
// perform AMC frame filtering, return true if frame is filtered
{
    ...
    */
    ...
    if (taskP.taskStatus(TP_mlme_scan_request))
    {
        ...
        else if (((taskP.mlme_scan_request_ScanType == ACTIVE_SCAN)     //Active scan or Passive scan, drop all except for data
    ...
    if (!mpib.macPromiscuousMode)
    {
        ...
        //check source PAN ID for data frame
        if ((frmType == Ieee802154_DATA)
        ...
        //check dest. PAN ID (data has no dest address fields)
...
void Ieee802154Mac::resetTRX()
{
    ...
    if ((mpib.macBeaconOrder != 15)||(rxBO != 15))  //data enabled
    {
    ...
    {
        EV << "[MAC]: non-data, wether go to sleep depending on parameter macRxOnWhenIdle" << endl;
        t_state = mpib.macRxOnWhenIdle?phy_RX_ON:phy_TRX_OFF;
    }
...
void Ieee802154Mac::handle_PLME_CCA_confirm(PHYenum status)
{
    if (taskP.taskStatus(TP_CCA_CSMACA))
    {
        ...
        {
            if (!beaconEnabled)                 // non-data, unslotted
            {
                tmpCsmaca = NULL;
                csmacaCallBack(phy_IDLE);           // unslotted successfully, callback
                return;
            }
            else                            // data-enabled, slotted
            {
                CW--;                       // Case C1, data-enabled, slotted
                if (CW == 0)                    // Case D1
                {
                    ...
                    else                    // Case E2: postpone until next data sent or received
                    {
                        CW = 2;
                        bPeriodsLeft = 0;
...
void Ieee802154Mac::handle_PLME_SET_TRX_STATE_confirm(PHYenum status)
{
    ...
    //transmit the packet
    if (beaconWaitingTx)        // periodically tx data
    {
        // to synchronize better, we don't transmit the data here
    }
    else if (txAck)
    {
        //although no CSMA-CA required for the transmission of ack.,
        //but we still need to locate the backoff period boundary if data enabled
        //(refer to page 157, line 25-31)
        if ((mpib.macBeaconOrder == 15)&&(rxBO == 15))  //non-data enabled
            delay = 0.0;
...
void Ieee802154Mac::csmacaEntry(char pktType)
{
    ...
    {
        waitBcnCmdAck = false;          // data packet not yet transmitted
        numBcnCmdRetry = 0;
...
void Ieee802154Mac::csmacaStart(bool firsttime, Ieee802154Frame* frame, bool ackReq)
{
    ...
    if (rxBO != 15)     // is receiving data
    {
        // set schedBcnRxTime: the latest time that the data should have arrived
        // schedBcnRxTime may not be bcnRxTime, when missing some beacons
        // or being in the middle of rxing a data
...
bool Ieee802154Mac::csmacaCanProceed(simtime_t wtime, bool afterCCA)
{
    ...
    if (!toParent(tmpCsmaca))       // as a coordinator
    {
        if (mpib.macBeaconOrder != 15)      // data enabled as a coordinator
        {
        ...
        else        // non-data as a coordinator, but data enabled as a device
            bPeriodsLeft = -1;
    }
    else        // as a device
    {
        if (rxBO != 15)     // data enabled as a device
        {
            // we assume that no data will be lost
            /* If max data loss reached, use unslotted version
            rxBI = rxSfSpec.BI / phy_symbolrate;
        ...
        else        // non-data as a device, but data enabled as a coordinator
            bPeriodsLeft = -1;
    }
    ...
    if (!ok)
    {
        ...
        // if as a device, need to track next data if it is not being tracked
        if (rxBO != 15)
    ...
    // check 3
    // If backoff can finish before the end of CAP or sent in non-data, calculate the time needed to finish the transaction and evaluate it
    ...
    {
        t_transacTime += Ieee802154Phy::aTurnaroundTime/phy_symbolrate;        //transceiver turn-around time (receiver may need to do this to transmit next data)
    ...
    {
        if (mpib.macBeaconOrder != 15)      // sent in data-enabled, check the end of tx cap
    ...
    {
        if (rxBO != 15)                 // sent in data-enabled, check the end of rx cap
        {
            t_fCAP = getFinalCAP('r');
        }
...
simtime_t Ieee802154Mac::getFinalCAP(char trxType)
{
    ...
    {
        if (mpib.macBeaconOrder != 15)  // data enabled
        {
    ...
    else                                                    // check rx CAP
    {
        if (rxBO != 15)                         // data enabled
        {
            ...
            tmpf = aMaxLostBeacons * rxBI;
            // adjust if data loss occurs or during rceiving bcn
            if ((rxCAP < now)&&(rxCAP + tmpf >= now))   //no more than <aMaxLostBeacons> beacons missed
            {
                EV << "[MAC]: during receiving a data, now is: " << now << ", last data is rceived at " << bcnRxTime << endl;
...
// To be called each time that a data received or transmitted if backoffStatus == 99
{
    simtime_t wtime;
    if (!txAck)
        PLME_SET_TRX_STATE_request(phy_RX_ON);
...
void Ieee802154Mac::startBcnTxTimer(bool txFirstBcn, simtime_t startTime)
{
    ...
    if (txFirstBcn)     // tx my first data
    {
        ASSERT(mpib.macBeaconOrder != 15);
        if (startTime == 0.0)   // transmit data right now
        {
...
// this function should be called when txing a new data is received in <handleBcnTxTimer()>
{
...
// this function should be called when a  new data is received in <handleBeacon()>
{
...
{
    if (rxBO != 15)     //data enabled (do nothing if data not enabled)
    {
        if (txAck)
        {
            delete txAck;
...
void Ieee802154Mac::handleBcnTxTimer()
{
    ...
    if (mpib.macBeaconOrder != 15)      //data enabled
    {
        if (!txNow_bcnTxTimer)          //enable the transmitter
        {
            ...
            if (txAck)      // transmitting data has highest priority
            {
                delete txAck;
                txAck = NULL;
            }
    ...
    startBcnTxTimer();  //don't disable this even data not enabled (data may be temporarily disabled like in channel scan, but it will be enabled again)
}
...
void Ieee802154Mac::taskSuccess(char type, bool csmacaRes)
{
    ...
    if (type == 'b')    //data
    {
        EV << "[MAC]: taskSuccess for sending data" << endl;
        /*if (!txBeacon)
        {
        ...
        if (t_CAP == 0)     // it's possible that there is no time left in current CAP after txing data
        {
            csmacaRes = false;
            PLME_SET_TRX_STATE_request(phy_TRX_OFF);
            EV << "[MAC]: no time left in current CAP after txing data, trying to turn off radio" << endl;
        }
        else
        {
            PLME_SET_TRX_STATE_request(phy_RX_ON);
            EV << "[MAC]: turn on radio receiver after txing data" << endl;
        }
        //CSMA-CA may be waiting for the new data
        if (backoffStatus == 99)
            csmacaTrxBeacon('t');
...
void Ieee802154Mac::dispatch(PHYenum pStatus, const char *frFunc, PHYenum req_state, MACenum mStatus)
{
    ...
    else if (strcmp(frFunc,"handle_PD_DATA_confirm") == 0)  // only process phy_SUCCESS here
    {
        if (txPkt == txBeacon)
        {
            {
                resetTRX();
                taskSuccess('b');   // data transmitted successfully
            }
        }
        else if (txPkt == txAck)
        {
...
int Ieee802154Mac::calFrmByteLength(Ieee802154Frame* frame)
{
    ...
    if (frmType == Ieee802154_DATA)       // Fig 37
    {
//        Ieee802154BeaconFrame* beaconFrm = check_and_cast<Ieee802154BeaconFrame *>(frame);
    ...
    else if (frmType == Ieee802154_CMD)
    {
        ...
        case    Ieee802154_DATA_REQUEST:
            byteLength = SIZE_OF_802154_DATA_REQUEST;
            break;
...
uint16_t Ieee802154Mac::gts_request_cmd(UINT_16 macShortAddr, uint16_t length, bool isReceive)
{
    ...
    gtsCount++;
    // new paramters put into effect when txing next data
    //isGtsUpdate = true;
    return  startSlot;
}
