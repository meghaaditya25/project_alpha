...
class INET_API Ieee802154Mac: public WirelessMacBase
{
  ...
  protected:
    ...
    */
     ...
     *  After receiving a data from the coordinator for the first time and associating with it,
    ...
    */
    ...
    virtual void    csmacaTrxBeacon(char trx);  // To be called each time that a data received or transmitted
...
// member variables
  ...
  protected:
    ...
    */
    ...
    /** @brief  pointer to the CommunicationUnit module */
    CommunicationUnit* mpNb;
    ...
    */
    ...
    /**
    * @name Data related variables
    */
    //@{
    /** @brief data order of incoming superframe */
    uint16_t rxBO;
    ...
    /** @brief duration (in backoff periods) of latest outgoing data */
    uint16_t txBcnDuration;

    /** @brief duration (in backoff periods) of latest incoming data */
    uint16_t rxBcnDuration;

    /** @brief length (in s) of a unit of backoff period, aUnitBackoffPeriod/phy_symbolrate */
    simtime_t bPeriod;

    /** @brief the time that the last data was received
     updated right after the data is received at MAC */
    simtime_t bcnRxTime;

    /** @brief the scheduled time that the latest data should have arrived
     sometimes bcnRxTime lags due to data loss (assumed impossible in the model) or in the middle of receiving a data,
    ...
    /** @brief flag for using data or not */
    bool beaconEnabled;

    /** @brief indicating a data frame waiting for transmission, suppress all other transmissions */
    ...
    */
    ...
    /** @brief store new value of final superframe slot in the CAP after updating GTS settings
     and put into effect when next data is transmitted  */
    ...
    */
    ...
    /** @brief buffer for data frames to be transmitted without CSMA/CA */
    Ieee802154Frame* txBeacon;

    /** @brief buffer for data or cmd frames coming from the upper layer,
     to be transmitted with CSMA/CA */
    Ieee802154Frame* txBcnCmdUpper;

    /** @brief buffer for data or cmd frames responding to receiving a packet,
     to be transmitted with CSMA/CA */
    ...
    */
    ...
    /** @brief timer for transmitting data periodically */
    cMessage* bcnTxTimer;

    /** @brief timer for timer for ACK timeout */
    cMessage* ackTimeoutTimer;

    /** @brief timer for locating backoff boundary before txing ACK if data-enabled */
    cMessage* txAckBoundTimer;

    /** @brief timer for locating backoff boundary before txing Cmd or data if data-enabled */
    ...
    */
    ...
    /** @brief true while a sent data or cmd frame in txBcnCmd is waiting for ACK */
    bool waitBcnCmdAck;

    /** @brief true while a sent data or cmd frame in txBcnCmdUpper is waiting for ACK */
    ...
    /** @brief number of retries for txing a data or cmd frame in txBcnCmd */
    uint16_t numBcnCmdRetry;

    /** @brief number of retries for txing a data or cmd frame in txBcnCmdUpper */
    ...
    /**
    * @name Link objects for data and data pkts duplication detection
    */
    //@{
