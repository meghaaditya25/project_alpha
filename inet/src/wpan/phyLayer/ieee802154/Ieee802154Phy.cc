...
uint16_t Ieee802154Phy::aMaxBeaconOverhead             = 75;       //max # of octets added by the MAC sublayer to the payload of its data frame
uint16_t Ieee802154Phy::aMaxBeaconPayloadLength = aMaxPHYPacketSize - aMaxBeaconOverhead;       //max size, in octets, of a data payload
...
void Ieee802154Phy::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        if (change)
        {
            aMaxBeaconPayloadLength = aMaxPHYPacketSize - aMaxBeaconOverhead;       //max size, in octets, of a data payload
...
void Ieee802154Phy::handleLowerMsgStart(AirFrame * airframe)
{
    ...
    else
    {
        ...
        if (snrInfo.ptr != NULL)
        {
            ...
            EV << "[PHY]: Discover the path to the Destination node\n";
            addNewSnr();
        }
        ...
        {
            EV << "[PHY]: noise level is high, Source Node: 44\n";
            setRadioState(RadioState::RECV);
        }
    }
}
