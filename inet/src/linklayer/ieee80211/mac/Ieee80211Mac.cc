...
Ieee80211Mac::~Ieee80211Mac()
{
    ...
    cancelAndDelete(endAttacker_Detection);
...
void Ieee80211Mac::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        EV<<" slotTime = "<<getSlotTime()*1e6<<"us Attacker_Detection = "<< getAttacker_Detection()*1e6<<"us";
        ...
        endAttacker_Detection = new cMessage("Transmission_of_packets");
        ...
        {
			int chV=par("ch");
			setEndAIFS(i, new cMessage("Transmission_of_packets", i));
...
{
    recordScalar("number of packets delivers", numReceived);
    recordScalar("number of car collisions", numCollision);
    recordScalar("number of car internal collisions", numInternalCollision);
    for (int i=0; i<numCategories(); i++)
    {
        std::stringstream os;
        os<< i;
...
void Ieee80211Mac::handleWithFSM(cMessage *msg)
{
    ...
    {
        if (fsm.getState() == WAITAIFS && endAttacker_Detection->isScheduled())
        {
            // a difs was schedule because all queues ware empty
            // change difs for aifs
            simtime_t remaint = getAIFS(currentAC)-getAttacker_Detection();
            scheduleAt(endAttacker_Detection->getArrivalTime()+remaint, endAIFS(currentAC));
            cancelEvent(endAttacker_Detection);
        }
...
// Special case, is  endTimeout ACK and the radio state  is RECV, the system must wait until end reception (9.3.2.8 ACK procedure)
    ...
    FSMA_Switch(fsm)
    {
        ...
        FSMA_State(WAITAIFS)
        {
            ...
            FSMA_Event_Transition(Attacker_Detection-Over,
                                  msg == endAttacker_Detection && transmissionQueueEmpty(),
                                  BACKOFF,
            ...
            FSMA_Event_Transition(Busy,
                                  ...
                                  if (endAttacker_Detection->isScheduled()) backoff(numCategories()-1) = true;
                                  cancelAIFSPeriod();
                                  );
...
simtime_t Ieee80211Mac::getAttacker_Detection()
{
...
{
// FIXME:   return getSIFS() + getAttacker_Detection() + (8 * ACKSize + aPreambleLength + aPLCPHeaderLength) / lowestDatarate;
    return getSIFS() + getAttacker_Detection() + controlFrameTxTime(LENGTH_ACK);
}
...
void Ieee80211Mac::scheduleAttacker_DetectionPeriod()
{
    if (lastReceiveFailed)
    {
        EV << "reception of last frame failed, scheduling EIFS period\n";
        scheduleAt(simTime() + getEIFS(), endAttacker_Detection);
    }
    else
    {
        EV << "scheduling Attacker_Detection period\n";
        scheduleAt(simTime() + getAttacker_Detection(), endAttacker_Detection);
    }
}

void Ieee80211Mac::cancelAttacker_DetectionPeriod()
{
    EV << "canceling Attacker_Detection period\n";
    cancelEvent(endAttacker_Detection);
}
...
{
    bool schedule = false;
    if (classifier == NULL) //OCB
    {
        currentAC = 0;
        if (!transmissionQueue(0)->empty() && !endAIFS(0)->isScheduled())
        {
            if (!endAttacker_Detection->isScheduled())
            {
                if (lastReceiveFailed)
                {
                    EV << "reception of last frame failed, scheduling EIFS period \n";
                ...
                {
                    EV << "scheduling Attacker_Detection period (frame pending)\n";
                    scheduleAt(simTime() + getAttacker_Detection(), endAIFS(0));
                }
            }
        }
        if (!endAIFS(0)->isScheduled() && !endAttacker_Detection->isScheduled())
        {
            // schedule default Attacker_Detection
            EV << "scheduling Attacker_Detection period (no frame pending)\n";
            scheduleAttacker_DetectionPeriod();
        }
        return;
    }
    ...
    for (int i = 0; i<numCategories(); i++)
    {
        if (!endAIFS(i)->isScheduled() && !transmissionQueue(i)->empty())
        {
            ...
            if (lastReceiveFailed)
            {
                EV << "reception of last frame failed, scheduling EIFS-Attacker_Detection+AIFS period (" << i << ")\n";
                scheduleAt(simTime() + getEIFS() - getAttacker_Detection() + getAIFS(i), endAIFS(i));
            }
            else
            {
    ...
    if (!schedule && !endAttacker_Detection->isScheduled())
    {
        // schedule default Attacker_Detection
        currentAC = numCategories()-1;
        scheduleAttacker_DetectionPeriod();
