...
void STP::handleBPDU(BPDU * bpdu)
{
    ...
    {
        if (port->getRole() == ieee80211acInterfaceData::DESIGNATED)
        {
    ...
    // BPDU from root
    else if (port->getRole() == ieee80211acInterfaceData::ROOT)
    {
...
void STP::generateTCN()
{
    ...
    {
        if (getPortInterfaceData(rootPort)->getRole() == ieee80211acInterfaceData::ROOT)
        {
...
{
    ieee80211acInterfaceData * port = getPortInterfaceData(portNum);
    ieee80211acInterfaceData * xBpdu = new ieee80211acInterfaceData();
...
void STP::setSuperiorBPDU(int portNum, BPDU * bpdu)
{
    ...
    ieee80211acInterfaceData * portData = getPortInterfaceData(portNum);
...
void STP::handleTick()
{
    ...
    for (unsigned int i = 0; i < numPorts; i++)
    {
        ...
        if (port->getRole() == ieee80211acInterfaceData::DISABLED)
            continue;
        ...
        if (port->getRole() == ieee80211acInterfaceData::ROOT || port->getRole() == ieee80211acInterfaceData::DESIGNATED)
        {
...
void STP::checkTimers()
{
    ...
    for (unsigned int i = 0; i < numPorts; i++)
    {
        ...
        if (port->getAge() >= currentMaxAge)
        {
            ...
            if (port->getRole() == ieee80211acInterfaceData::ROOT)
            {
                initInterfacedata(i);
                lostRoot();
            }
        ...
        if (port->getRole() == ieee80211acInterfaceData::ROOT || port->getRole() == ieee80211acInterfaceData::DESIGNATED)
        {
            if (port->getFdWhile() >= currentFwdDelay)
            {
                ...
                case ieee80211acInterfaceData::DISCARDING:
                    ...
                    port->setState(ieee80211acInterfaceData::LEARNING);
                    port->setFdWhile(0);
                    break;
                case ieee80211acInterfaceData::LEARNING:
                    EV_DETAIL << "Port=" << i << " goes into forwarding state." << endl;
                    port->setState(ieee80211acInterfaceData::FORWARDING);
                    port->setFdWhile(0);
                    break;
                default:
                    port->setFdWhile(0);
...
int STP::comparePorts(ieee80211acInterfaceData * portA, ieee80211acInterfaceData * portB)
{
    int result;
...
void STP::selectRootPort()
{
    ...
    int result;
    ieee80211acInterfaceData * best = getPortInterfaceData(0);
    ieee80211acInterfaceData * currentPort;

    for (unsigned int i = 0; i < numPorts; i++)
    {
        currentPort = getPortInterfaceData(i);
        currentPort->setRole(ieee80211acInterfaceData::NOTASSIGNED);
    ...
    rootPort = xRootPort;
    getPortInterfaceData(rootPort)->setRole(ieee80211acInterfaceData::ROOT);
...
void STP::selectDesignatedPorts()
{
    ...
    desPorts.clear();
    ieee80211acInterfaceData * port;
    ieee80211acInterfaceData * bridgeGlobal = new ieee80211acInterfaceData();
    int result;
    ...
    for (unsigned int i = 0; i < numPorts; i++)
    {
        ...
        if (port->getRole() == ieee80211acInterfaceData::ROOT || port->getRole() == ieee80211acInterfaceData::DISABLED)
            continue;
        ...
        if (result > 0)
        {
            ...
            port->setRole(ieee80211acInterfaceData::DESIGNATED);
            continue;
        }
        if (result < 0)
        {
            EV_DETAIL<< "Port=" << i << " goes into alternate role." << endl;
            port->setRole(ieee80211acInterfaceData::ALTERNATE);
            continue;
        }
    }
    delete bridgeGlobal;
...
void STP::initInterfacedata(unsigned int portNum)
{
    ...
    ifd->setRole(ieee80211acInterfaceData::NOTASSIGNED);
    ifd->setState(ieee80211acInterfaceData::DISCARDING);
