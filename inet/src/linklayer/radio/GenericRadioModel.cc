...
void GenericRadioModel::initializeFrom(cModule *radioModule)
{
    ...
    else if (strcmp(modulationName, "QAM")==0)
        modulation = new QAMModulation();
    else if (strcmp(modulationName, "QPSK")==0)
        modulation = new QPSKModulation();
    else
...
PhyIndication GenericRadioModel::isReceivedCorrectly(AirFrame *airframe, const SnrList& receivedList)
{
    // calculate snirMin
