...
void HwmpProtocol::initialize(int stage)
{
    if (stage == 4)
    {
        ...
        WATCH(nextProactive);
        Ieee80211p * etx = dynamic_cast<Ieee80211p *>(interface80211ptr->getEstimateCostProcess(0));
        if (etx == NULL)
            useEtxProc=false;
        else
            useEtxProc=true;
...
uint32_t HwmpProtocol::GetLinkMetric(const MACAddress &peerAddress)
{
    ...
    if (useEtxProc)
    {
        Ieee80211p * etx = dynamic_cast<Ieee80211p *>(interface80211ptr->getEstimateCostProcess(0));if (etx)
