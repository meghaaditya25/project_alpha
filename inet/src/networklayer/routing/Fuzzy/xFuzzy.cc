...
Define_Module(Fuzzy::xFuzzy);

#define Fuzzy_EV EV << "Fuzzy at " << getHostName() << " "
...
void xFuzzy::initialize(int stage)
{
    ...
    else if (stage == 4)
    {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (slash)
            {
                ...
                if (pLength < 0 || pLength > Kconnect)
                    throw cRuntimeError("invalid prefix length in 'clientAddresses' parameter: '%s/%s'", clientAddress, slash);
                Kconnect = pLength;
            }
           // clientAddressAndKconnectPairs.push_back(std::pair<IPv4Address, int>(address.get4(), Kconnect));
        }
    }
    else if (stage == 5)
    {
...
void xFuzzy::handleMessage(cMessage * message)
{
    if (!isNodeUp())
...
void xFuzzy::processSelfMessage(cMessage * message)
{
...
void xFuzzy::processMessage(cMessage * message)
{
...
void xFuzzy::startRouteDiscovery(const IPv4Address & target)
{
    Fuzzy_EV << "Starting route discovery: originator = " << getSelfAddress() << ", target = " << target << endl;
...
void xFuzzy::retryRouteDiscovery(const IPv4Address & target, int retryCount)
{
    Fuzzy_EV << "Retrying route discovery: originator = " << getSelfAddress() << ", target = " << target << ", retry = " << retryCount << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
    sendRREQ(createRREQ(target, retryCount));
    scheduleRREQWaitRREPTimer(createRREQWaitRREPTimer(target, retryCount));
}

void xFuzzy::completeRouteDiscovery(const IPv4Address & target)
{
    Fuzzy_EV << "Completing route discovery: originator = " << getSelfAddress() << ", target = " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
...
void xFuzzy::cancelRouteDiscovery(const IPv4Address & target)
{
    Fuzzy_EV << "Canceling route discovery: originator = " << getSelfAddress() << ", target = " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
    std::multimap<IPv4Address, IPv4Datagram *>::iterator lt = targetAddressToDelayedPackets.lower_bound(target);
    std::multimap<IPv4Address, IPv4Datagram *>::iterator ut = targetAddressToDelayedPackets.upper_bound(target);
    for (std::multimap<IPv4Address, IPv4Datagram *>::iterator it = lt; it != ut; it++)
...
bool xFuzzy::hasOngoingRouteDiscovery(const IPv4Address & target)
{
...
void xFuzzy::delayDatagram(IPv4Datagram * datagram)
{
    Fuzzy_EV << "Queuing datagram: source = " << datagram->getSrcAddress() << ", destination = " << datagram->getDestAddress() << endl;
...
void xFuzzy::reinjectDelayedDatagram(IPv4Datagram * datagram)
{
    Fuzzy_EV << "Sending queued datagram: source = " << datagram->getSrcAddress() << ", destination = " << datagram->getDestAddress() << endl;
    networkProtocol->reinjectQueuedDatagram(const_cast<const IPv4Datagram *>(datagram));
}

void xFuzzy::dropDelayedDatagram(IPv4Datagram * datagram)
{
    Fuzzy_EV << "Dropping queued datagram: source = " << datagram->getSrcAddress() << ", destination = " << datagram->getDestAddress() << endl;
    networkProtocol->dropQueuedDatagram(const_cast<const IPv4Datagram *>(datagram));
}

void xFuzzy::eraseDelayedDatagrams(const IPv4Address & target)
{
    std::multimap<IPv4Address, IPv4Datagram *>::iterator lt = targetAddressToDelayedPackets.lower_bound(target);
    std::multimap<IPv4Address, IPv4Datagram *>::iterator ut = targetAddressToDelayedPackets.upper_bound(target);
...
bool xFuzzy::hasDelayedDatagrams(const IPv4Address & target)
{
...
void xFuzzy::cancelRREQTimer(const IPv4Address & target)
{
    std::map<IPv4Address, RREQTimer *>::iterator tt = targetAddressToRREQTimer.find(target);
    cancelEvent(tt->second);
}

void xFuzzy::deleteRREQTimer(const IPv4Address & target)
{
    std::map<IPv4Address, RREQTimer *>::iterator tt = targetAddressToRREQTimer.find(target);
    delete tt->second;
}

void xFuzzy::eraseRREQTimer(const IPv4Address & target)
{
    std::map<IPv4Address, RREQTimer *>::iterator tt = targetAddressToRREQTimer.find(target);
...
RREQWaitRREPTimer * xFuzzy::createRREQWaitRREPTimer(const IPv4Address & target, int retryCount)
{
...
void xFuzzy::scheduleRREQWaitRREPTimer(RREQWaitRREPTimer * message)
{
    Fuzzy_EV << "Scheduling RREQ wait RREP timer" << endl;
    targetAddressToRREQTimer[message->getTarget()] = message;
    scheduleAt(simTime() + routeRREQWaitTime, message);
}

void xFuzzy::processRREQWaitRREPTimer(RREQWaitRREPTimer * message)
{
    Fuzzy_EV << "Processing RREQ wait RREP timer" << endl;
...
RREQBackoffTimer * xFuzzy::createRREQBackoffTimer(const IPv4Address & target, int retryCount)
{
...
void xFuzzy::scheduleRREQBackoffTimer(RREQBackoffTimer * message)
{
    Fuzzy_EV << "Scheduling RREQ backoff timer" << endl;
    targetAddressToRREQTimer[message->getTarget()] = message;
    scheduleAt(simTime() + computeRREQBackoffTime(message->getRetryCount()), message);
}

void xFuzzy::processRREQBackoffTimer(RREQBackoffTimer * message)
{
    Fuzzy_EV << "Processing RREQ backoff timer" << endl;
    retryRouteDiscovery(message->getTarget(), message->getRetryCount() + 1);
    delete message;
}

simtime_t xFuzzy::computeRREQBackoffTime(int retryCount)
{
...
RREQHolddownTimer * xFuzzy::createRREQHolddownTimer(const IPv4Address & target)
{
...
void xFuzzy::scheduleRREQHolddownTimer(RREQHolddownTimer * message)
{
    Fuzzy_EV << "Scheduling RREQ holddown timer" << endl;
    targetAddressToRREQTimer[message->getTarget()] = message;
    scheduleAt(simTime() + rreqHolddownTime, message);
}

void xFuzzy::processRREQHolddownTimer(RREQHolddownTimer * message)
{
    Fuzzy_EV << "Processing RREQ holddown timer" << endl;
    const IPv4Address & target = message->getTarget();
    eraseRREQTimer(target);
...
void xFuzzy::sendUDPPacket(UDPPacket * packet, double delay)
{
    if (delay == 0)
        send(packet, "ipOut");
    else
...
void xFuzzy::processUDPPacket(UDPPacket * packet)
{
...
void xFuzzy::sendFuzzyPacket(FuzzyPacket * packet, const InterfaceEntry * interfaceEntry, const IPv4Address & nextHop, double delay)
{
    ...
    udpPacket->setSourcePort(Fuzzy_UDP_PORT);
    udpPacket->setDestinationPort(Fuzzy_UDP_PORT);
    udpPacket->setControlInfo(dynamic_cast<cObject *>(networkProtocolControlInfo));
    sendUDPPacket(udpPacket, delay);
}

void xFuzzy::processFuzzyPacket(FuzzyPacket * packet)
{
    if (dynamic_cast<RREQ *>(packet))
        ...
        throw cRuntimeError("Unknown Fuzzy packet");
}
...
bool xFuzzy::permissibleRteMsg(RteMsg * rteMsg)
{
   ...
   // if (dynamic_cast<RREQ *>(rteMsg) && (!originatorNode.getlreliable() || !originatorNode.getHasSequenceNumber()))
        //return false;
  //  else if (dynamic_cast<RREP *>(rteMsg) && (!targetNode.getlreliable() || !targetNode.getHasSequenceNumber()))
        //return false;
...
void xFuzzy::processRteMsg(RteMsg * rteMsg)
{
    // 7.5. Handling a Received RteMsg
...
int xFuzzy::computeRteMsgBitLength(RteMsg * rteMsg)
{
...
RREQ * xFuzzy::createRREQ(const IPv4Address & target, int retryCount)
{
    ...
    std::map<IPv4Address, FuzzySequenceNumber>::iterator st = targetAddressToSequenceNumber.find(target);
    ...
    //originatorNode.setlreliable(true);
   // originatorNode.setrveloci(0);
  //  originatorNode.setMetricType(true);
    originatorNode.setMetricType(HOP_COUNT);
  //  targetNode.setMetricType(true);
...
void xFuzzy::sendRREQ(RREQ * rreq)
{
    ...
    Fuzzy_EV << "Sending RREQ: originator = " << originator << ", target = " << target << endl;
    sendFuzzyPacket(rreq, NULL, IPv4Address::LL_MANET_ROUTERS, uniform(0, maxJitter).dbl());
}

void xFuzzy::processRREQ(RREQ * rreqIncoming)
{
    ...
    Fuzzy_EV << "Processing RREQ: originator = " << originator << ", target = " << target << endl;
    if (permissibleRteMsg(rreqIncoming)) {
        ...
        if (isClientAddress(target)) {
            ...
            Fuzzy_EV << "Received RREQ for client: originator = " << originator << ", target = " << target << endl;
            if (useMulticastRREP)
            ...
            Fuzzy_EV << "Forwarding RREQ: originator = " << originator << ", target = " << target << endl;
        ...
        Fuzzy_EV << "Dropping non-permissible RREQ" << endl;
    delete rreqIncoming;
}

int xFuzzy::computeRREQBitLength(RREQ * rreq)
{
...
RREP * xFuzzy::createRREP(RteMsg * rteMsg)
{
    return createRREP(rteMsg, NULL);
}

RREP * xFuzzy::createRREP(RteMsg * rteMsg, IPv4Route * route)
{
    QLearnData * routeData = check_and_cast<QLearnData *>(route->getProtocolData());
    ...
    //targetNode.setlreliableType(true);
    targetNode.setMetricType(routeData->getMetricType());
    // 9. RREP.Metric[TargNode] := Route[TargNode].Metric
   // targetNode.setlreliable(true);
  //  targetNode.setrveloci(route->getMetric());
...
void xFuzzy::sendRREP(RREP * rrep)
{
    ...
    Fuzzy_EV << "Sending broadcast RREP: originator = " << originator << ", target = " << target << endl;
    sendFuzzyPacket(rrep, NULL, IPv4Address::LL_MANET_ROUTERS, 0);
}

void xFuzzy::sendRREP(RREP * rrep, IPv4Route * route)
{
    const IPv4Address & target = rrep->getTargetNode().getAddress();
    const IPv4Address & originator = rrep->getOriginatorNode().getAddress();
    const IPv4Address & nextHop = route->getGateway();
    rrep->setBitLength(computeRREPBitLength(rrep));
    Fuzzy_EV << "Sending unicast RREP: originator = " << originator << ", target = " << target << ", nextHop = " << nextHop << endl;
    sendFuzzyPacket(rrep, route->getInterface(), nextHop, 0);
}

void xFuzzy::processRREP(RREP * rrepIncoming)
{
    ...
    Fuzzy_EV << "Processing RREP: originator = " << originator << ", target = " << target << endl;
    if (permissibleRteMsg(rrepIncoming)) {
        ...
        if (isClientAddress(originator)) {
            Fuzzy_EV << "Received RREP for client: originator = " << originator << ", target = " << target << endl;
            ...
            Fuzzy_EV << "Forwarding RREP: originator = " << originator << ", target = " << target << endl;
            ...
            if (useMulticastRREP)
                ...
                if (route)
                    ...
                    Fuzzy_EV << "No route found toward originator, dropping RREP: originator = " << originator << ", target = " << target << endl;
            }
        }
    }
    else
...
int xFuzzy::computeRREPBitLength(RREP * rrep)
{
...
RERR * xFuzzy::createRERR(std::vector<IPv4Address> & unreachableAddresses)
{
    RERR * rerr = new RERR();
    for (int i = 0; i < (int)unreachableAddresses.size(); i++) {
        ...
        addressBlock->setKconnect(32);
        addressBlock->setHasValidityTime(false);
      //  addressBlock->setlreliable(false);
     //   addressBlock->setMetricType(false);
        std::map<IPv4Address, FuzzySequenceNumber>::iterator st = targetAddressToSequenceNumber.find(unreachableAddress);
...
void xFuzzy::sendRERR(RERR * rerr)
{
    rerr->setBitLength(computeRERRBitLength(rerr));
    Fuzzy_EV << "Sending RERR: unreachableNodeCount = " << rerr->getUnreachableNodeArraySize() << endl;
    sendFuzzyPacket(rerr, NULL, IPv4Address::LL_MANET_ROUTERS, 0);
}

void xFuzzy::sendRERRForUndeliverablePacket(const IPv4Address & destination)
{
    Fuzzy_EV << "Sending RERR for undeliverable packet: destination = " << destination << endl;
...
void xFuzzy::sendRERRForBrokenLink(const InterfaceEntry * interfaceEntry, const IPv4Address & nextHop)
{
    Fuzzy_EV << "Sending RERR for broken link: nextHop = " << nextHop << endl;
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getSource() == this) {
            QLearnData * routeData = check_and_cast<QLearnData *>(route->getProtocolData());
            FuzzyRouteState routeState = getRouteState(routeData);
            if (routeState != BROKEN && route->getInterface() == interfaceEntry && route->getGateway() == nextHop)
            {
                Fuzzy_EV << "Marking route as broken: " << route << endl;
    ...
    if (unreachableAddresses.size() == 0)
        Fuzzy_EV << "No unreachable address found" << endl;
    else
        sendRERR(createRERR(unreachableAddresses));
}

void xFuzzy::processRERR(RERR * rerrIncoming)
{
    Fuzzy_EV << "Processing RERR" << endl;
...
int xFuzzy::computeRERRBitLength(RERR * rerr)
{
    // TODO: validityTime, metric, metricType, TLVs
    // 1. <address-block> := <num-addr> <addr-flags> (<head-length><head>?)? (<tail-length><tail>?)? <mid>* <prefix-length>*
    int addressBlock = 8 + 8;
...
void xFuzzy::updateRoutes(RteMsg * rteMsg, AddressBlock & addressBlock)
{
    ...
    if (address == getSelfAddress())
        ...
        for (int i = 0; i < routingTable->getNumRoutes(); i++) {
            ...
            if (routeCandidate->getSource() == this) {
                QLearnData *routeData = check_and_cast<QLearnData *>(routeCandidate->getProtocolData());
        ...
        if (!route) {
            ...
            Fuzzy_EV << "Adding new route: " << route << endl;
           ...
           // if ((addressBlock.getSequenceNumber() > routeData->getSequenceNumber()) ||
               ...
               // (routeData->getBroken() && isLoopFree(rteMsg, route)))
            {
                ...
                Fuzzy_EV << "Updating existing route: " << route << endl;
                updateRoute(rteMsg, addressBlock, route);
                Fuzzy_EV << "Route updated: " << route << endl;
                routingTable->addRoute(route);
            }
        }
    }
...
IPv4Route * xFuzzy::createRoute(RteMsg * rteMsg, AddressBlock & addressBlock)
{
    IPv4Route * route = new IPv4Route();
  //  route->setSourceType(IPv4Route::Fuzzy);
    route->setSourceType(IPv4Route::DYMO);
    route->setSource(this);
    route->setProtocolData(new QLearnData());
    updateRoute(rteMsg, addressBlock, route);
    return route;
}

void xFuzzy::updateRoute(RteMsg * rteMsg, AddressBlock & addressBlock, IPv4Route * route)
{
    ...
    FuzzySequenceNumber sequenceNumber = addressBlock.getSequenceNumber();
    ...
    // note that Fuzzy packets are not routed on the IP level, so we can use the source address here
    ...
    //if (addressBlock.getlreliableType())
...
// TODO: use
int xFuzzy::getLinkCost(const InterfaceEntry * interfaceEntry, FuzzyMetricType metricType)
{
    switch (metricType) {
        case HOP_COUNT:
            return 1;
...
bool xFuzzy::isLoopFree(RteMsg * rteMsg, IPv4Route * route)
{
    // TODO: implement
    return true;
}
...
void xFuzzy::processExpungeTimer()
{
    Fuzzy_EV << "Processing expunge timer" << endl;
    expungeRoutes();
    scheduleExpungeTimer();
}

void xFuzzy::scheduleExpungeTimer()
{
    Fuzzy_EV << "Scheduling expunge timer" << endl;
...
void xFuzzy::expungeRoutes()
{
    Fuzzy_EV << "Expunging routes from routing table: routeCount = " << routingTable->getNumRoutes() << endl;
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getSource() == this) {
            ...
            {
                Fuzzy_EV << "Expunging route: " << route << endl;
...
simtime_t xFuzzy::getNextExpungeTime()
{
...
FuzzyRouteState xFuzzy::getRouteState(QLearnData * routeData)
{
...
bool xFuzzy::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

void xFuzzy::configureInterfaces()
{
...
std::string xFuzzy::getHostName()
{
    return host->getFullName();
}

IPv4Address xFuzzy::getSelfAddress()
{
    return routingTable->getRouterId();
}

bool xFuzzy::isClientAddress(const IPv4Address & address)
{
    if (routingTable->isLocalAddress(address))
        return true;
    else {
        ////for (std::vector<std::pair<IPv4Address, int> >::iterator it = clientAddressAndKconnectPairs.begin(); it != clientAddressAndKconnectPairs.end(); it++)
...
void xFuzzy::addSelfNode(RteMsg * rteMsg)
{
    ...
    //addressBlock->setlreliable(true);
    //addressBlock->setrveloci(0);
    //addressBlock->setMetricType(true);
...
void xFuzzy::addNode(RteMsg * rteMsg, AddressBlock & addressBlock)
{
...
void xFuzzy::incrementSequenceNumber()
{
...
INetfilter::IHook::Result xFuzzy::ensureRouteForDatagram(IPv4Datagram * datagram)
{
    ...
    if (destination.isMulticast() || destination.isLimitedBroadcastAddress() || routingTable->isLocalAddress(destination))
        ...
        Fuzzy_EV << "Finding route: source = " << source << ", destination = " << destination << endl;
        IPv4Route * route = routingTable->findBestMatchingRoute(destination);
        QLearnData * routeData = route ? dynamic_cast<QLearnData *>(route->getProtocolData()) : NULL;
        bool broken = routeData && routeData->getBroken();
        if (route && !route->getGateway().isUnspecified() && !broken) {
            Fuzzy_EV << "Route found: source = " << source << ", destination = " << destination << ", route: " << route << endl;
            if (routeData)
            ...
            Fuzzy_EV << (broken ? "Broken" : "Missing") << " route: source = " << source << ", destination = " << destination << endl;
            delayDatagram(datagram);
            if (!hasOngoingRouteDiscovery(destination))
                startRouteDiscovery(destination);
            else
                Fuzzy_EV << "Route discovery is in progress: originator = " << getSelfAddress() << ", target = " << destination << endl;
            return QUEUE;
        }
        else
...
bool xFuzzy::handleOperationStage(LifecycleOperation * operation, int stage, IDoneCallback * doneCallback)
{
    Enter_Method_Silent();
...
void xFuzzy::receiveChangeNotification(int signalID, const cObject *obj)
{
    Enter_Method("receiveChangeNotification");
    if (signalID == NF_LINK_BREAK) {
        Fuzzy_EV << "Received link break" << endl;
