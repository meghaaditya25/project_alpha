...
Define_Module(GTLQR::xGTLQR);

#define GTLQR_EV EV << "GTLQR at " << getHostName() << " "
...
void xGTLQR::initialize(int stage)
{
    cSimpleModule::initialize(stage);

    if (stage == 0)
    {
        // GTLQR parameters from RFC
...
void xGTLQR::handleMessage(cMessage * message)
{
    if (!isNodeUp())
...
void xGTLQR::processSelfMessage(cMessage * message)
{
...
void xGTLQR::processMessage(cMessage * message)
{
...
void xGTLQR::startRouteDiscovery(const IPv4Address & target)
{
    GTLQR_EV << "Starting route discovery: originator = " << getSelfAddress() << ", target = " << target << endl;
...
void xGTLQR::retryRouteDiscovery(const IPv4Address & target, int retryCount)
{
    GTLQR_EV << "Retrying route discovery: originator = " << getSelfAddress() << ", target = " << target << ", retry = " << retryCount << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
    sendRREQ(createRREQ(target, retryCount));
    scheduleRREQWaitRREPTimer(createRREQWaitRREPTimer(target, retryCount));
}

void xGTLQR::completeRouteDiscovery(const IPv4Address & target)
{
    GTLQR_EV << "Completing route discovery: originator = " << getSelfAddress() << ", target = " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
...
void xGTLQR::cancelRouteDiscovery(const IPv4Address & target)
{
    GTLQR_EV << "Canceling route discovery: originator = " << getSelfAddress() << ", target = " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
    std::multimap<IPv4Address, IPv4Datagram *>::iterator lt = targetAddressToDelayedPackets.lower_bound(target);
    std::multimap<IPv4Address, IPv4Datagram *>::iterator ut = targetAddressToDelayedPackets.upper_bound(target);
    for (std::multimap<IPv4Address, IPv4Datagram *>::iterator it = lt; it != ut; it++)
...
bool xGTLQR::hasOngoingRouteDiscovery(const IPv4Address & target)
{
...
void xGTLQR::delayDatagram(IPv4Datagram * datagram)
{
    GTLQR_EV << "Queuing datagram: source = " << datagram->getSrcAddress() << ", destination = " << datagram->getDestAddress() << endl;
...
void xGTLQR::reinjectDelayedDatagram(IPv4Datagram * datagram)
{
    GTLQR_EV << "Sending queued datagram: source = " << datagram->getSrcAddress() << ", destination = " << datagram->getDestAddress() << endl;
    networkProtocol->reinjectQueuedDatagram(const_cast<const IPv4Datagram *>(datagram));
}

void xGTLQR::dropDelayedDatagram(IPv4Datagram * datagram)
{
    GTLQR_EV << "Dropping queued datagram: source = " << datagram->getSrcAddress() << ", destination = " << datagram->getDestAddress() << endl;
    networkProtocol->dropQueuedDatagram(const_cast<const IPv4Datagram *>(datagram));
}

void xGTLQR::eraseDelayedDatagrams(const IPv4Address & target)
{
    std::multimap<IPv4Address, IPv4Datagram *>::iterator lt = targetAddressToDelayedPackets.lower_bound(target);
    std::multimap<IPv4Address, IPv4Datagram *>::iterator ut = targetAddressToDelayedPackets.upper_bound(target);
...
bool xGTLQR::hasDelayedDatagrams(const IPv4Address & target)
{
...
void xGTLQR::cancelRREQTimer(const IPv4Address & target)
{
    std::map<IPv4Address, RREQTimer *>::iterator tt = targetAddressToRREQTimer.find(target);
    cancelEvent(tt->second);
}

void xGTLQR::deleteRREQTimer(const IPv4Address & target)
{
    std::map<IPv4Address, RREQTimer *>::iterator tt = targetAddressToRREQTimer.find(target);
    delete tt->second;
}

void xGTLQR::eraseRREQTimer(const IPv4Address & target)
{
    std::map<IPv4Address, RREQTimer *>::iterator tt = targetAddressToRREQTimer.find(target);
...
RREQWaitRREPTimer * xGTLQR::createRREQWaitRREPTimer(const IPv4Address & target, int retryCount)
{
...
void xGTLQR::scheduleRREQWaitRREPTimer(RREQWaitRREPTimer * message)
{
    GTLQR_EV << "Scheduling RREQ wait RREP timer" << endl;
    targetAddressToRREQTimer[message->getTarget()] = message;
    scheduleAt(simTime() + routeRREQWaitTime, message);
}

void xGTLQR::processRREQWaitRREPTimer(RREQWaitRREPTimer * message)
{
    GTLQR_EV << "Processing RREQ wait RREP timer" << endl;
...
RREQBackoffTimer * xGTLQR::createRREQBackoffTimer(const IPv4Address & target, int retryCount)
{
...
void xGTLQR::scheduleRREQBackoffTimer(RREQBackoffTimer * message)
{
    GTLQR_EV << "Scheduling RREQ backoff timer" << endl;
    targetAddressToRREQTimer[message->getTarget()] = message;
    scheduleAt(simTime() + computeRREQBackoffTime(message->getRetryCount()), message);
}

void xGTLQR::processRREQBackoffTimer(RREQBackoffTimer * message)
{
    GTLQR_EV << "Processing RREQ backoff timer" << endl;
    retryRouteDiscovery(message->getTarget(), message->getRetryCount() + 1);
    delete message;
}

simtime_t xGTLQR::computeRREQBackoffTime(int retryCount)
{
...
RREQHolddownTimer * xGTLQR::createRREQHolddownTimer(const IPv4Address & target)
{
...
void xGTLQR::scheduleRREQHolddownTimer(RREQHolddownTimer * message)
{
    GTLQR_EV << "Scheduling RREQ holddown timer" << endl;
    targetAddressToRREQTimer[message->getTarget()] = message;
    scheduleAt(simTime() + rreqHolddownTime, message);
}

void xGTLQR::processRREQHolddownTimer(RREQHolddownTimer * message)
{
    GTLQR_EV << "Processing RREQ holddown timer" << endl;
    const IPv4Address & target = message->getTarget();
    eraseRREQTimer(target);
...
void xGTLQR::sendUDPPacket(UDPPacket * packet, double delay)
{
    if (delay == 0)
        send(packet, "ipOut");
    else
...
void xGTLQR::processUDPPacket(UDPPacket * packet)
{
...
//
// handling GTLQR packets
//

void xGTLQR::sendGTLQRPacket(GTLQRPacket * packet, const InterfaceEntry * interfaceEntry, const IPv4Address & nextHop, double delay)
{
    ...
    udpPacket->setSourcePort(GTLQR_UDP_PORT);
    udpPacket->setDestinationPort(GTLQR_UDP_PORT);
    udpPacket->setControlInfo(dynamic_cast<cObject *>(networkProtocolControlInfo));
    sendUDPPacket(udpPacket, delay);
}

void xGTLQR::processGTLQRPacket(GTLQRPacket * packet)
{
    if (dynamic_cast<RREQ *>(packet))
        ...
        throw cRuntimeError("Unknown GTLQR packet");
}
...
bool xGTLQR::permissibleRteMsg(RteMsg * rteMsg)
{
...
void xGTLQR::processRteMsg(RteMsg * rteMsg)
{
    // 7.5. Handling a Received RteMsg
...
int xGTLQR::computeRteMsgBitLength(RteMsg * rteMsg)
{
...
RREQ * xGTLQR::createRREQ(const IPv4Address & target, int retryCount)
{
    ...
    std::map<IPv4Address, GTLQRSequenceNumber>::iterator st = targetAddressToSequenceNumber.find(target);
...
void xGTLQR::sendRREQ(RREQ * rreq)
{
    ...
    GTLQR_EV << "Sending RREQ: originator = " << originator << ", target = " << target << endl;
    sendGTLQRPacket(rreq, NULL, IPv4Address::LL_MANET_ROUTERS, uniform(0, maxJitter).dbl());
}

void xGTLQR::processRREQ(RREQ * rreqIncoming)
{
    ...
    GTLQR_EV << "Processing RREQ: originator = " << originator << ", target = " << target << endl;
    if (permissibleRteMsg(rreqIncoming)) {
        ...
        if (isClientAddress(target)) {
            ...
            GTLQR_EV << "Received RREQ for client: originator = " << originator << ", target = " << target << endl;
            if (useMulticastRREP)
            ...
            GTLQR_EV << "Forwarding RREQ: originator = " << originator << ", target = " << target << endl;
        ...
        GTLQR_EV << "Dropping non-permissible RREQ" << endl;
    delete rreqIncoming;
}

int xGTLQR::computeRREQBitLength(RREQ * rreq)
{
...
RREP * xGTLQR::createRREP(RteMsg * rteMsg)
{
    return createRREP(rteMsg, NULL);
}

RREP * xGTLQR::createRREP(RteMsg * rteMsg, IPv4Route * route)
{
    SKHData * routeData = check_and_cast<SKHData *>(route->getProtocolData());
...
void xGTLQR::sendRREP(RREP * rrep)
{
    ...
    GTLQR_EV << "Sending broadcast RREP: originator = " << originator << ", target = " << target << endl;
    sendGTLQRPacket(rrep, NULL, IPv4Address::LL_MANET_ROUTERS, 0);
}

void xGTLQR::sendRREP(RREP * rrep, IPv4Route * route)
{
    const IPv4Address & target = rrep->getTargetNode().getAddress();
    const IPv4Address & originator = rrep->getOriginatorNode().getAddress();
    const IPv4Address & nextHop = route->getGateway();
    rrep->setBitLength(computeRREPBitLength(rrep));
    GTLQR_EV << "Sending unicast RREP: originator = " << originator << ", target = " << target << ", nextHop = " << nextHop << endl;
    sendGTLQRPacket(rrep, route->getInterface(), nextHop, 0);
}

void xGTLQR::processRREP(RREP * rrepIncoming)
{
    ...
    GTLQR_EV << "Processing RREP: originator = " << originator << ", target = " << target << endl;
    if (permissibleRteMsg(rrepIncoming)) {
        ...
        if (isClientAddress(originator)) {
            GTLQR_EV << "Received RREP for client: originator = " << originator << ", target = " << target << endl;
            ...
            GTLQR_EV << "Forwarding RREP: originator = " << originator << ", target = " << target << endl;
            ...
            if (useMulticastRREP)
                ...
                if (route)
                    ...
                    GTLQR_EV << "No route found toward originator, dropping RREP: originator = " << originator << ", target = " << target << endl;
            }
        }
    }
    else
...
int xGTLQR::computeRREPBitLength(RREP * rrep)
{
...
RERR * xGTLQR::createRERR(std::vector<IPv4Address> & unreachableAddresses)
{
    RERR * rerr = new RERR();
    for (int i = 0; i < (int)unreachableAddresses.size(); i++) {
        ...
        std::map<IPv4Address, GTLQRSequenceNumber>::iterator st = targetAddressToSequenceNumber.find(unreachableAddress);
...
void xGTLQR::sendRERR(RERR * rerr)
{
    rerr->setBitLength(computeRERRBitLength(rerr));
    GTLQR_EV << "Sending RERR: unreachableNodeCount = " << rerr->getUnreachableNodeArraySize() << endl;
    sendGTLQRPacket(rerr, NULL, IPv4Address::LL_MANET_ROUTERS, 0);
}

void xGTLQR::sendRERRForUndeliverablePacket(const IPv4Address & destination)
{
    GTLQR_EV << "Sending RERR for undeliverable packet: destination = " << destination << endl;
...
void xGTLQR::sendRERRForBrokenLink(const InterfaceEntry * interfaceEntry, const IPv4Address & nextHop)
{
    GTLQR_EV << "Sending RERR for broken link: nextHop = " << nextHop << endl;
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getSource() == this) {
            SKHData * routeData = check_and_cast<SKHData *>(route->getProtocolData());
            GTLQRRouteState routeState = getRouteState(routeData);
            if (routeState != BROKEN && route->getInterface() == interfaceEntry && route->getGateway() == nextHop)
            {
                GTLQR_EV << "Marking route as broken: " << route << endl;
    ...
    if (unreachableAddresses.size() == 0)
        GTLQR_EV << "No unreachable address found" << endl;
    else
        sendRERR(createRERR(unreachableAddresses));
}

void xGTLQR::processRERR(RERR * rerrIncoming)
{
    GTLQR_EV << "Processing RERR" << endl;
...
int xGTLQR::computeRERRBitLength(RERR * rerr)
{
    // TODO: validityTime, metric, metricType, TLVs
    // 1. <address-block> := <num-addr> <addr-flags> (<head-length><head>?)? (<tail-length><tail>?)? <mid>* <prefix-length>*
    int addressBlock = 8 + 8;
...
void xGTLQR::updateRoutes(RteMsg * rteMsg, AddressBlock & addressBlock)
{
    ...
    if (address == getSelfAddress())
        ...
        for (int i = 0; i < routingTable->getNumRoutes(); i++) {
            ...
            if (routeCandidate->getSource() == this) {
                SKHData *routeData = check_and_cast<SKHData *>(routeCandidate->getProtocolData());
        ...
        if (!route) {
            ...
            GTLQR_EV << "Adding new route: " << route << endl;
                ...
                (routeData->getBroken() && isLoopFree(rteMsg, route)))
            {
                ...
                GTLQR_EV << "Updating existing route: " << route << endl;
                updateRoute(rteMsg, addressBlock, route);
                GTLQR_EV << "Route updated: " << route << endl;
                routingTable->addRoute(route);
            }
        }
    }
...
IPv4Route * xGTLQR::createRoute(RteMsg * rteMsg, AddressBlock & addressBlock)
{
    IPv4Route * route = new IPv4Route();
  //  route->setSourceType(IPv4Route::GTLQR);
    route->setSourceType(IPv4Route::DYMO);
    route->setSource(this);
    route->setProtocolData(new SKHData());
    updateRoute(rteMsg, addressBlock, route);
    return route;
}

void xGTLQR::updateRoute(RteMsg * rteMsg, AddressBlock & addressBlock, IPv4Route * route)
{
    ...
    GTLQRSequenceNumber sequenceNumber = addressBlock.getSequenceNumber();
    ...
    // note that GTLQR packets are not routed on the IP level, so we can use the source address here
...
// TODO: use
int xGTLQR::getLinkCost(const InterfaceEntry * interfaceEntry, GTLQRMetricType metricType)
{
    switch (metricType) {
        case HOP_COUNT:
            return 1;
...
bool xGTLQR::isLoopFree(RteMsg * rteMsg, IPv4Route * route)
{
    // TODO: implement
    return true;
}
...
void xGTLQR::processExpungeTimer()
{
    GTLQR_EV << "Processing expunge timer" << endl;
    expungeRoutes();
    scheduleExpungeTimer();
}

void xGTLQR::scheduleExpungeTimer()
{
    GTLQR_EV << "Scheduling expunge timer" << endl;
...
void xGTLQR::expungeRoutes()
{
    GTLQR_EV << "Expunging routes from routing table: routeCount = " << routingTable->getNumRoutes() << endl;
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getSource() == this) {
            ...
            {
                GTLQR_EV << "Expunging route: " << route << endl;
...
simtime_t xGTLQR::getNextExpungeTime()
{
...
GTLQRRouteState xGTLQR::getRouteState(SKHData * routeData)
{
...
bool xGTLQR::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

void xGTLQR::configureInterfaces()
{
...
std::string xGTLQR::getHostName()
{
    return host->getFullName();
}

IPv4Address xGTLQR::getSelfAddress()
{
    return routingTable->getRouterId();
}

bool xGTLQR::isClientAddress(const IPv4Address & address)
{
...
void xGTLQR::addSelfNode(RteMsg * rteMsg)
{
...
void xGTLQR::addNode(RteMsg * rteMsg, AddressBlock & addressBlock)
{
...
void xGTLQR::incrementSequenceNumber()
{
...
INetfilter::IHook::Result xGTLQR::ensureRouteForDatagram(IPv4Datagram * datagram)
{
    ...
    if (destination.isMulticast() || destination.isLimitedBroadcastAddress() || routingTable->isLocalAddress(destination))
        ...
        GTLQR_EV << "Finding route: source = " << source << ", destination = " << destination << endl;
        IPv4Route * route = routingTable->findBestMatchingRoute(destination);
        SKHData * routeData = route ? dynamic_cast<SKHData *>(route->getProtocolData()) : NULL;
        bool broken = routeData && routeData->getBroken();
        if (route && !route->getGateway().isUnspecified() && !broken) {
            GTLQR_EV << "Route found: source = " << source << ", destination = " << destination << ", route: " << route << endl;
            if (routeData)
            ...
            GTLQR_EV << (broken ? "Broken" : "Missing") << " route: source = " << source << ", destination = " << destination << endl;
            delayDatagram(datagram);
            if (!hasOngoingRouteDiscovery(destination))
                startRouteDiscovery(destination);
            else
                GTLQR_EV << "Route discovery is in progress: originator = " << getSelfAddress() << ", target = " << destination << endl;
            return QUEUE;
        }
        else
...
bool xGTLQR::handleOperationStage(LifecycleOperation * operation, int stage, IDoneCallback * doneCallback)
{
    Enter_Method_Silent();
...
void xGTLQR::receiveChangeNotification(int signalID, const cObject *obj)
{
    Enter_Method("receiveChangeNotification");
    if (signalID == NF_LINK_BREAK) {
        GTLQR_EV << "Received link break" << endl;
