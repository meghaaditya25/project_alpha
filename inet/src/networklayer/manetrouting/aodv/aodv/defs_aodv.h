...
#define AODV_LOG_PATH_PREFIX "aodv-"
...
/*
  NS_PORT: TEMPORARY SOLUTION: Moved the two variables into the AODV class,
  and placed the function definition after the AODV class definition.

  (This is to avoid running several passes through defs.h during the source
  code extraction performed by the AODV class.)
...
typedef struct
{
    ...
    /* NS_PORT: Additions for the AODV packet type in ns-2 */
#ifdef NS_PORT
...
#define HDR_AODV(p) ((hdr_aodvuu *) hdr_aodvuu::access(p))
#else
} AODV_msg;
#endif
