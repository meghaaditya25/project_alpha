...
// We need this because we want to know which CommunicationUnit the notification comes from
// (INotifiable::receiveChangeNotification() doesn't have CommunicationUnit* as arg).
class RoutingTableRecorderListener : public INotifiable
{
  ...
  public:
    RoutingTableRecorderListener(RoutingTableRecorder *recorder, CommunicationUnit *nb) :  nb(nb), recorder(recorder) {}
...
#endif
    {
        CommunicationUnit *nb = dynamic_cast<CommunicationUnit *>(simulation.getModule(id));
        if (nb)
        {
...
void RoutingTableRecorder::receiveChangeNotification(CommunicationUnit *nb, int category, const cObject *details)
{
